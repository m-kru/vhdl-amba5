...
package axi_stream is
  ...
  -- Stream type with data width of 64 bits.
  type stream64_t is record
    data   : data64_t;
  ...
  function init (
    data   : data64_t := (others => '0');
    strb   : std_logic_vector(7 downto 0) := (others => '1');
    keep   : std_logic_vector(7 downto 0) := (others => '1');
    user   : std_logic_vector(7 downto 0) := (others => '-');
    valid  : std_logic := '0';
    last   : std_logic := '0';
    wakeup : std_logic := '1';
    id     : std_logic_vector(7 downto 0) := (others => '-');
...
package body axi_stream is
  ...
  function to_stream8 (s64 : stream64_t) return stream8_t is
    constant s : stream8_t := (
      s64.data(7 downto 0),
      s64.strb(0 downto 0),
      s64.keep(0 downto 0),
  ...
  function to_stream16 (s64 : stream64_t) return stream16_t is
    constant s : stream16_t := (
      s64.data(15 downto 0),
      s64.strb( 1 downto 0),
      s64.keep( 1 downto 0),
      s64.user( 1 downto 0),
      s64.valid,
  ...
  function to_stream32 (s64 : stream64_t) return stream32_t is
    constant s : stream32_t := (
      s64.data(31 downto 0),
      s64.strb( 3 downto 0),
      s64.keep( 3 downto 0),
      s64.user( 3 downto 0),
      s64.valid,
  ...
  function init (
    ...
    id     : std_logic_vector(7 downto 0) := (others => '-');
    dest   : std_logic_vector(7 downto 0) := (others => '-')
  ) return stream64_t is
    constant s : stream64_t := (data, strb, keep, user, valid, last, wakeup, id, dest);
  begin
    return s;
  end function;


  function to_stream64 (s8 : stream8_t) return stream64_t is
    variable s : stream64_t := init(
      data => (others => '-'),
      strb => (others => '-'),
      keep => (others => '-'),
      user => (others => '-')
  ...
  function to_stream64 (s16 : stream16_t) return stream64_t is
    variable s : stream64_t := init(
      data => (others => '-'),
      strb => (others => '-'),
      keep => (others => '-'),
  ...
  function to_stream64 (s32 : stream32_t) return stream64_t is
    variable s : stream64_t := init(
      data => (others => '-'),
      strb => (others => '-'),
      keep => (others => '-'),
  ...
  function to_stream1024 (s64 : stream64_t) return stream1024_t is
    variable s : stream1024_t := init(
      data => (others => '-'),
      strb => (others => '-'),
      keep => (others => '-'),
