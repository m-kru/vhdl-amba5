...
package packet_dropper is
  ...
  type packet_dropper_t is record
    ...
    -- Internal elements
    state : state_t;
    ostream_next : stream1024_t; -- Output stream
  end record;
  ...
  function init (
    ...
    state         : state_t := IDLE;
    ostream_next  : stream1024_t := STREAM_INIT
...
package body packet_dropper is
  ...
  function init (
    ...
    constant pd : packet_dropper_t := (
      make(REPORT_PREFIX), ostream, iready, drop_event, state, ostream_next
    );
  begin
    return pd;
  end function;
  ...
  begin
    if drop = '1' then
      ...
      if pd.stream_next.valid = '1' then
        pd.ostream := pd.ostream_next;
        pd.iready  := oready;
        pd.state   := FORWARDING;
      end if;
    end if;
    ...
    if pd.ostream_next.valid and oready = '1' then
  ...
  begin
    ...
    if pd.ostream.valid = '1' and oready = '1' then
      pd.ostream := pd.ostream_next;
      -- Check packet end condition
      if pd.ostream.last = '1' then
        pd.state := IDLE;
      end if;
    end if;
